...
const cfgName = `${appName
  .substring(0, 1)
  .toLocaleLowerCase()}${appName.substring(1)}`;
    ...
    fileTriggers?.forEach((fileTrigger: FileTrigger) => {
      ...
      if (!file) {
        outputChannel.appendLine(
          'Invalid entry in fileTriggers configuration: file property required.'
        );
        return;
      }
      ...
      watcher.onDidChange((uri: vscode.Uri) =>
        handleFileChange(fileTrigger, uri, 'onChange')
      );
      watcher.onDidCreate((uri: vscode.Uri) =>
        handleFileChange(fileTrigger, uri, 'onCreate')
      );
      watcher.onDidDelete((uri: vscode.Uri) =>
        handleFileChange(fileTrigger, uri, 'onDelete')
      );
      ...
      outputChannel.appendLine(
        `Watching [${pattern.baseUri}, ${pattern.pattern}] - (${[
          !ignoreCreateEvents ? 'create' : '',
          !ignoreChangeEvents ? 'change' : '',
          !ignoreDeleteEvents ? 'delete' : '',
        ]
          .filter(Boolean)
          .join(', ')})`
      );
...
function handleFileChange(
  ...
  if (!command) {
    vscode.window.showInformationMessage(`${file} has changed.`);
    return;
  }

  if (autoExecute) {
    vscode.window.showInformationMessage(
      `${file} has changed. Running \`${command}\`.`
    );
    if (commandType === 'task') {
...
function runCommand(
  ...
  try {
    ...
    terminal.show();
    outputChannel.appendLine(
      `Running command: ${command} for ${file} at ${uri} on ${eventName}`
    );
  } catch (error) {
  ...
  vscode.tasks.fetchTasks().then((availableTasks: vscode.Task[]) => {
    availableTasks.forEach((task: vscode.Task) => {
      if (task.name === command) {
        outputChannel.appendLine(
          `Running task: ${command} for ${file} at ${uri} on ${eventName}`
        );
